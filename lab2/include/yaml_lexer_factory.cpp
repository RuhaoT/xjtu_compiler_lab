...
// Load DFA configurations
std::unordered_map<std::string, std::unique_ptr<DFA>> LoadDFAConfigs(const std::string& specific_config) {
    // Load file
    try {
        ...
        std::unordered_map<std::string, std::unique_ptr<DFA>> dfa_mapping;
        ...
        for (const auto& dfa : config["dfas"]) {
            ...
            dfa_mapping[dfa_name] = std::make_unique<DFA>(dfa_config);
        }
...
void CheckDFAConfigurations(const std::unordered_map<std::string, std::unique_ptr<DFA>>& dfa_mapping) {
    // iterate through each DFA in the mapping
    for (const auto& dfa_pair : dfa_mapping) {
        const std::string& dfa_name = dfa_pair.first;
        const DFA& dfa = *dfa_pair.second;
...
void CheckTokenDFARelationships(const std::unordered_set<TokenType>& token_types, const std::unordered_map<std::string, std::unique_ptr<DFA>>& dfa_mapping) {
