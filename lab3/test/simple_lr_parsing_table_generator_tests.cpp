...
TEST_F(SimpleLRParsingTableGeneratorTests, TestGenerateParsingTable)
{
    ...
    std::string filename = "test/data/simple_lr_parsing_table_generator/minimal_correct_cfg.yml";
...
{
    // Load the CFG from the YAML file
    std::string filename = "test/data/simple_lr_parsing_table_generator/complicated_cfg_1_with_conflict.yml";
    cfg_model::CFG cfg = load_test_cfg(filename);
