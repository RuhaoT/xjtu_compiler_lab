...
ItemSetToParsingTable::ItemSetToParsingTable(const lr_parsing_model::ItemSet &item_set)
{
    ...
    try
    {
        ...
        generate_dfa();
        build_parsing_table();
        generate_itemset_parsing_table_mapping();
    }
...
lr_parsing_model::LRParsingTableGenerationResult ItemSetToParsingTable::generate_itemset_parsing_table_mapping()
{
    try{
        // check prerequisites
        // item_set, nfa, dfa, their mappings, and parsing table
        if (item_set.items.empty())
        {
            std::string error_msg = "Error: Item set is empty";
            spdlog::error(error_msg);
        ...
        if (parsing_table.all_states.empty())
        {
            std::string error_msg = "Error: Parsing table is empty";
            spdlog::error(error_msg);
            throw std::runtime_error(error_msg);
        }
        ...
        for (const auto &item : item_set.items)
        {
            ...
            std::unordered_set<std::string> dfa_state_names = item_set_dfa_mapping.item_set_to_dfa_state[item];
        ...
        item_set_parsing_table_mapping = new_item_set_parsing_table_mapping;
        spdlog::debug("ItemSet parsing table mapping generated successfully");
        return {parsing_table, item_set_parsing_table_mapping};

    }
    catch (const std::exception &e)
    {
        std::string error_msg = "Error during ItemSet parsing table generation: " + std::string(e.what());
        spdlog::error(error_msg);
        throw std::runtime_error(error_msg);
    }
}
